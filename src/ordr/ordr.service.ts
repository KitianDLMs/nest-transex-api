...
export class OrdrService {
  ...
  private async getJwt(): Promise<string> {
    try {
      const response = await axios.post(
        'http://200.27.177.162:90/ApiSamtech/api/login/authenticate',
        this.authBody,
  ...
  async getProgramaPorPedido(order_code: string, order_date: string) {
    ...
    const url = 'http://200.27.177.162:90/ApiSamtech/api/programa/por_pedido';
  ...
  async getTicketsPorPedido(order_code: string, order_date: string) {
    const url = 'http://200.27.177.162:90/ApiSamtech/api/tick/por_pedido';
    try {
      const token = await this.getJwt();
  ...
  async getPedidosPorProyectoExterno(projCode: string, custCode: string) {
    const url = 'http://200.27.177.162:90/ApiSamtech/api/pedido/pedido_proyecto';
    try {
      const token = await this.getJwt();
  ...
  async getPedidosFuturosPorProyectoExterno(projCode: string, custCode: string) {
    const url = 'http://200.27.177.162:90/ApiSamtech/api/pedido/futuro';                 
    try {
      const token = await this.getJwt();
  ...
  async   getPedidosPorCliente(custCode: string) {
    ...
    const url =
      'http://200.27.177.162:90/ApiSamtech/api/pedido/pedido_cliente';
